...
function useEffectImpl(fiberEffectTag, hookEffectTag, create, inputs) {
  ...
  if (currentHook !== null) {
    ...
    var prevInputs = prevEffect.inputs;
    nextEffect = pushEffect(inputsAreEqual(nextInputs, prevInputs) ? NoEffect$1 : hookEffectTag, create, prevEffect.destroy, nextInputs);
  } else {
...
function flushPassiveEffectsBeforeSchedulingUpdateOnFiber(fiber) {
...
function commitRoot(root, finishedWork) {
  ...
  if (firstEffect !== null && rootWithPendingPassiveEffects !== null) {
    ...
    var resolvedFirstEffect = firstPassiveEffect = firstEffect;

    var passiveEffectCallback = void 0;
    if (enableSchedulerTracing) {
      // TODO: Avoid this extra callback by mutating the tracing ref directly,
      // like we do at the beginning of commitRoot. I've opted not to do that
      // here because that code is still in flux.
      passiveEffectCallback = tracing.unstable_wrap(function () {
        commitPassiveEffects(root, resolvedFirstEffect);
      });
    } else {
      passiveEffectCallback = commitPassiveEffects.bind(null, root, resolvedFirstEffect);
    }
    passiveEffectCallbackHandle = scheduler.unstable_scheduleCallback(passiveEffectCallback);
  }
...
function scheduleRootUpdate(current$$1, element, expirationTime, callback) {
  ...
  flushPassiveEffectsBeforeSchedulingUpdateOnFiber(current$$1);
